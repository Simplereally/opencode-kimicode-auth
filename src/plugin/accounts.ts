...
export class AccountManager {
  ...
  constructor(authFallback?: OAuthAuthDetails, stored?: AccountStorageV4 | null) {
    const authParts: RefreshParts | null = authFallback ? { refreshToken: authFallback.refresh } : null;
    ...
    if (authFallback && this.accounts.length > 0) {
      const authParts: RefreshParts = { refreshToken: authFallback.refresh };
    ...
    if (authFallback) {
      const parts: RefreshParts = { refreshToken: authFallback.refresh };
