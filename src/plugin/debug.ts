 ...
 */
export function initializeDebug(config: KimicodeConfig): void {
 ...
 */
export function startKimicodeDebugRequest(meta: KimicodeDebugRequestMeta): KimicodeDebugContext | null {
 ...
 */
export function logKimicodeDebugResponse(
  context: KimicodeDebugContext | null | undefined,
  response: Response,
  meta: KimicodeDebugResponseMeta = {},
): void {
  const state = getDebugState();
  ...
  logDebug(
    `[Kimicode Debug ${context.id}] Response ${response.status} ${response.statusText} (${durationMs}ms)`,
  );
  logDebug(
    `[Kimicode Debug ${context.id}] Response Headers: ${JSON.stringify(
  ...
  if (meta.body) {
    logDebug(
      `[Kimicode Debug ${context.id}] Response Body Preview: ${truncateForLog(meta.body)}`,
    );
  }
}
...
export function logRateLimitSnapshot(
  family: string,
  accounts: Array<{ index: number; email?: string; rateLimitResetTimes?: { kimi?: number } }>,
): void {
  if (!getDebugState().debugEnabled) return;
  const now = Date.now();
  const entries = accounts.map((account) => {
    const label = account.email ? account.email : `Account ${account.index + 1}`;
    const reset = account.rateLimitResetTimes?.[family as "kimi"];
