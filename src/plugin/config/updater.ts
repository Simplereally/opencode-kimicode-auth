...
function isLegacyKimicodeModelDefinition(value: unknown, expectedName: string): boolean {
...
export async function updateOpencodeConfig(
  ...
  try {
    ...
    const hasPlugin = config.plugin.some((p) =>
      p.includes("opencode-kimicode-auth")
    );
    if (!hasPlugin) {
    ...
    const existingModels =
      config.provider.moonshotai.models && typeof config.provider.moonshotai.models === "object"
        ? (config.provider.moonshotai.models as Record<string, unknown>)
        : {};
    ...
    const cleanedModels: Record<string, unknown> = { ...existingModels };
    if (
      isLegacyKimicodeModelDefinition(cleanedModels["kimi-for-coding"], "Kimi for Coding")
    ) {
      delete cleanedModels["kimi-for-coding"];
    }
    if (
      isLegacyKimicodeModelDefinition(cleanedModels["kimi-k2.5"], "Kimi K2.5")
    ) {
      delete cleanedModels["kimi-k2.5"];
    }
    if (
      isLegacyKimicodeModelDefinition(cleanedModels["kimi-k2-thinking"], "Kimi K2 Thinking")
    ) {
